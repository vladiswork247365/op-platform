...
SYS = (
    ...
    "→ talking_head.\n"
    "5а) ВСТАВКИ (ЭКРАНКИ/ПАНЕЛЬ/ИНТЕРФЕЙС, лицо:нет): если в каталоге есть ГОВОРЯЩАЯ ГОЛОВА "
    "(человек/ИИ-аватар, лицо:да) — она ОСНОВА ролика и держит контакт. Записи экрана/панели/"
    "продукта — это ВСТАВКИ-ДОКАЗАТЕЛЬСТВА: врезай их ТОЧЕЧНО на тех битах, где слова ПРЯМО про "
    "то, что на экране (анализ, оценка, цифры, отчёт, уведомление, дашборд, CRM), на 1.5–3с, и "
    "возвращайся на говорящего. НЕ ставь экранку на личные/эмоциональные фразы и НЕ давай ей "
    "забить весь ролик (обычно 20–40% времени максимум). Длинную экранку режь на РАЗНЫЕ куски "
    "(разные in/out) под разные фразы — показывай именно ту часть панели, о которой речь.\n"
    "6) НЕ ОБЯЗАТЕЛЬНО использовать ВСЕ клипы из каталога. Бери только те, что усиливают "
    "смысл. Слабые, размытые, не по теме, тёмные, дубли — ПРОПУСКАЙ. Лучше переиспользовать "
    "сильный клип с другой обрезкой, чем воткнуть слабый ради галочки.\n"
    "7) motion: для видео — punch|zoomin|zoomout|kick; для фото — kenburns_in|kenburns_out "
